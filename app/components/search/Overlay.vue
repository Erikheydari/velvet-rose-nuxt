...
<template>
  ...
  <Transition enter-active-class="transition-all duration-300 ease-out"
    ...
    <div v-if="isActive" ref="searchContainerRef" class="
        fixed md:absolute 
        inset-x-0 bottom-0 md:top-full md:bottom-auto md:left-0 md:right-0 
        z-50 
        bg-background 
        rounded-t-2xl
        p-4 md:px-8
      ...
      <div class="flex items-center gap-2 mb-4">
        ...
        <!-- Search button -->
        <TheButton variant="default" size="icon-lg" class="h-12 w-12 shrink-0 touch-manipulation" @click="handleSubmit"
          :disabled="!localQuery.trim()" aria-label="جستجو">
          <Search class="text-primary-foreground w-5 h-5" />
        </TheButton>

        <!-- Close button -->
        <TheButton variant="ghost" size="icon-lg" class="h-12 w-12 shrink-0 touch-manipulation" @click="handleClose"
          aria-label="بستن">
          <X class="w-5 h-5" />
        </TheButton>
      </div>
