<template>
  <div class="h-full lg:hero-height lg:h-screen  px-4 lg:px-12 pt-28">
    <center class="mb-8">
      <TheButton variant="outline" class="pr-7!" size="lg">
        بازگشت
        <ChevronLeft />
      </TheButton>
    </center>

    <div class="flex flex-col lg:flex-row justify-between w-full gap-6 h-full">
      <div class="basis-full lg:basis-1/3 flex h-full flex-col sticky gap-8">
        ...
        <!-- Options -->
        <ul v-if="productsStore.product?.options" class="mb-4">
          <li v-for="option in productsStore.product?.options.slice(0, 3)" :key="option"
            class="body-2 text-primary list-inside list-disc">
            {{ option }}
          </li>
          <li v-if="isOptionsExpanded" v-for="option in productsStore.product?.options.slice(3)" :key="option"
            class="body-2 text-primary list-inside list-disc">
            {{ option }}
          </li>
          <li class="body-2 text-primary">
            <TheButton variant="ghost" class="cursor-pointer py-0! mt-2"
              ...
              <span v-if="!isOptionsExpanded">
                {{ productsStore.product?.options.length - 3 }}
                مورد دیگر
              </span>
              <span v-else>
                بستن
      ...
      <div class="basis-full lg:basis-2/3">
        <FantastyHeading v-if="productsStore.product?.brand?.name || productsStore.product?.brand?.en_name"
          ...
          :description="productsStore.product?.full_name" titleClass="heading-2 font-extrabold lg:h-[8vh]"
          descriptionClass="heading-4 font-light! mt-4 pb-2" />

        <figure class="w-full aspect-square h-auto max-w-[60%] mx-auto">
...
const initializeSelections = () => {
  // Start with nothing selected so user has to click
...
const canAddToCart = computed(() => {
  ...
  // Require explicit user selection if colors/sizes exist
...
const handleAddToCart = async () => {
  ...
  try {
    ...
    // Success feedback (you can customize this)
    validationMessage.value = '';
...
watch(() => productsStore.product, (newProduct) => {
  if (newProduct) {
